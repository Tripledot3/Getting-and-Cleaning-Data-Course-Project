## Reading all the files I need into R
...
## Creating a function that will search through the "prime" file
## and find the the mean and standard deviation included both ID's
...
##Now we need to take this variable and put it in a subset that
## includes all the TRUE values
Prime_meanandstd <- Prime[,mean_and_std == TRUE]
## Now we want to add the activity name to the new dataset by merging
## it with the activity label using the activityId
...
## Order the data first by subjectId then ActivityId
Prime_clean <- Prime_clean[order(Prime_clean$subjectId, Prime_clean$activityId),]
# upload your data set as a txt file created with write.table()
